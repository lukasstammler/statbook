...
5. Verwerfe die Nullhypothese, wenn ein Konfidenzintervall für eine Differenz zwischen zwei Parametern den Wert 0 enthält.     
...
Wir verwenden hier für die Berechnung des 95%-Konfidenzintervalls den t-Wert für die Anzahl Freiheitsgrade df = (100 - 1) = 99. Am einfachsten finden Sie den t-Wert mit dem [Internet-Rechner Students T-Verteilung](http://eswf.uni-koeln.de/glossar/surfstat/t.htm).
...
Die Teststatistik für unsere Punktschätzung ist $t = 1.549$. Den p-Wert können wir wieder mit dem [Internet-Rechner Students T-Verteilung](http://eswf.uni-koeln.de/glossar/surfstat/t.htm) oder mit `R/jamovi` berechnen. 
...
3. Berechnung: Wenn die Voraussetzungen erfüllt sind, berechne SE und finde $t_{df}$, um die Grenzen des Konfidenzintervalls zu berechnen.   
...
Datensatz berechnet hat, kann der Einstichproben-T-Test durchgeführt werden. Beide 
...
$$CI_{95} = 181.6 \pm 1.976 \times 121.4 = [-62.4; 425.6]$$
```{r}
...
Die Anzahl Freiheitsgrade $df = n_{ns} + n_{s} - 2 = 100 + 50 - 2 = 148$: Wir können den p-Wert für $t$ wieder in einer Tabelle nachschlagen oder mit `R/jamovi`berechnen: 

```{r pvalue-births, echo=TRUE}
### R-Code

2 * (1 - pt(1.496, df = 148)) # p-Wert für eine zweiseitige Hypothese und df = 148
```

$p = 0.137$; Dieser $p$-Wert ist grösser als $\alpha = 0.05$ und wir haben keine ausreichende Evidenz, um die Nullhypothese zu verwerfen.
...
# write_csv(ss, file = "./data/07_schwangerschaft.csv")
